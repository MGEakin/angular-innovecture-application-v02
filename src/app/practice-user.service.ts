...
import { PracticeUser } from './practice-user';
...
export class PracticeUserService {
  ...
  private practiceUsersUrl = 'api/practiceUsers';  // URL to web api
  ...
  // getPracticeUsers(userId: number): Observable<PracticeUser[]> {
  //   const url = `${this.practiceUsersUrl}/?userId=${userId}`;
  //   return this.http.get<PracticeUser[]>(url)
  //     .pipe(
  //       tap(_ => this.log(`fetched practiceUsers`)),
  //       catchError(this.handleError<PracticeUser[]>('getPracticeUsers', []))
  //     );
  // }

  /** GET Users which belong to a Practices from the server */
  getPracticeUsers(practiceId: number): Observable<PracticeUser[]> {
    const url = `${this.practiceUsersUrl}/?practiceId=${practiceId}`;
    return this.http.get<PracticeUser[]>(url)
      .pipe(
        tap(_ => this.log(`fetched practiceUsers`)),
        catchError(this.handleError<PracticeUser[]>(`getPracticeUsers with url=${url}`, []))
      );
  }
